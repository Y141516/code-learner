...
function LessonView({ lang, level, lesson, onBack }) {
  ...
  const runCode = () => {
    ...
    // Simulate execution — capture console.log for JS, basic for others
    let output = '';
    try {
      if (lang.id === 'javascript') {
        const logs = [];
        const fc = { log:(...a)=>logs.push(a.map(x=>typeof x==='object'?JSON.stringify(x):String(x)).join(' ')), error:(...a)=>logs.push('[Error] '+a.join(' ')), warn:(...a)=>logs.push('[Warn] '+a.join(' ')) };
        // eslint-disable-next-line no-new-func
        const fn = new Function('console', userCode);
        fn(fc);
        output = logs.join('\n');
      } else {
        // For Python/HTML/etc — simulate by extracting expected patterns
        output = userCode;
      }
    } catch(e) {
    ...
    setCodeChecked(true);
    if (!output.includes('[Error]') && challenge.validate(output)) {
      if (!xpAwarded) { completeLesson(lesson.id, lang.id, level); setXpAwarded(true); setJustDone(true); }
    }
  };
            ...
            <div style={s.challengeCard}>
              ...
              {challenge?.type==='code' && (
                <div style={s.terminalWrap}>
                  ...
                  {codeChecked && (
                    <div style={{...s.codeVerdict, background:codeOutput&&challenge.validate&&challenge.validate(codeOutput)?'rgba(16,185,129,0.08)':'rgba(239,68,68,0.08)', borderColor:codeOutput&&challenge.validate&&challenge.validate(codeOutput)?'rgba(16,185,129,0.4)':'rgba(239,68,68,0.4)'}} className="anim-fade-in">
                      {codeOutput&&challenge.validate&&challenge.validate(codeOutput)
                        ...
                        : <><span style={{fontSize:24}}>🔍</span><div><strong style={{color:'#f87171'}}>Output mismatch.</strong><div style={{color:'#94a3b8',fontSize:13,marginTop:2}}>Check your logic and try again. {challenge.expectedOutput?`Expected: ${challenge.expectedOutput}`:''}</div></div></>
                      }
                    </div>
                  )}
                </div>
