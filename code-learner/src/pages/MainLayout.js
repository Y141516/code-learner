...
import CodeExperimentTab from './CodeExperimentTab';
import CelebrationModal from '../components/CelebrationModal';

const TABS = [
  { id:'lessons', label:'Coding Lessons',  icon:'📚', ready:true  },
  { id:'code',    label:'Code Experiment', icon:'⚡', ready:true  },
...
const BADGE_META = {
  ...
  lesson_5:      { icon:'📖', label:'5 Lessons'      },
  lesson_15:     { icon:'🏆', label:'15 Lessons'     },
};
...
function ComingSoon() {
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:'80vh',gap:20}}>
      ...
      <h2 style={{fontFamily:'Syne,sans-serif',fontSize:28,fontWeight:800,color:'#e8edf5'}}>Coming Soon</h2>
      <p style={{color:'#64748b',fontSize:16}}>This tab is being built — check back soon!</p>
    </div>
  );
}

export default function MainLayout() {
  const { user, logout, xp, level, xpInLevel, xpToNext, badges, streak, activeTab, setActiveTab, celebration, dismissCelebration } = useApp();
  ...
  const renderTab = () => {
    switch (activeTab) {
      ...
      case 'code':    return <CodeExperimentTab />;
    ...
    <div style={s.app}>
      ...
      <aside style={s.sidebar}>
        ...
        <div style={s.brand}>
          ...
          <div><div style={s.brandName}>Code Learner</div><div style={s.brandSub}>Tab 1 Active</div></div>
        </div>
        ...
        <nav style={s.nav}>
          {TABS.map(tab=>{
            ...
            const soon   = !tab.ready;
            return (
              <button key={tab.id} onClick={()=>handleTab(tab)} title={soon?'Coming soon!':tab.label}
                style={{...s.navBtn,...(active?s.navActive:{}),...(soon?s.navSoon:{})}}>
        ...
        <div style={s.xpCard}>
          ...
          <div style={s.xpBar}><div style={{...s.xpFill,width:`${Math.min((xpInLevel/200)*100,100)}%`}}/></div>
          <div style={s.statsRow}>
            <div style={s.stat}><span style={s.statVal}>🔥 {streak}</span><span style={s.statLbl}>Streak</span></div>
            <div style={s.statDiv}/>
            <div style={s.stat}><span style={s.statVal}>🏅 {badges.length}</span><span style={s.statLbl}>Badges</span></div>
          </div>
          {badges.length > 0 && (
            <div style={s.badges}>
...
const s = {
  ...
  logo:{width:44,height:44,borderRadius:13,background:'linear-gradient(135deg,#6366f1,#8b5cf6)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,boxShadow:'0 0 20px rgba(99,102,241,0.4)',flexShrink:0},
  ...
  navBtn:{display:'flex',alignItems:'center',gap:11,padding:'11px 14px',borderRadius:11,color:'#64748b',fontSize:14,fontWeight:500,fontFamily:'DM Sans,sans-serif',textAlign:'left',position:'relative',transition:'all 0.2s',width:'100%',border:'none',cursor:'pointer',background:'none'},
  ...
  navLabel:{flex:1},
  soonTag:{fontSize:10,padding:'2px 7px',background:'rgba(245,158,11,0.15)',border:'1px solid rgba(245,158,11,0.3)',borderRadius:20,color:'#f59e0b',fontWeight:600},
  ...
  xpLevel:{width:38,height:38,borderRadius:10,background:'linear-gradient(135deg,#f59e0b,#ef4444)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:800,flexShrink:0,color:'#fff'},
  xpMeta:{flex:1},
  ...
  avatar:{width:34,height:34,borderRadius:'50%',background:'linear-gradient(135deg,#6366f1,#8b5cf6)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,fontWeight:700,flexShrink:0,color:'#fff'},
  ...
  menu:{position:'absolute',bottom:'calc(100% + 8px)',left:0,right:0,background:'#0d1117',border:'1px solid #1a2740',borderRadius:12,padding:'8px',zIndex:200,boxShadow:'0 8px 32px rgba(0,0,0,0.5)'},
  menuItem:{padding:'8px 10px',fontSize:13,color:'#94a3b8'},
  menuDiv:{height:1,background:'#1a2740',margin:'4px 0'},
  logoutBtn:{width:'100%',padding:'9px 10px',color:'#f87171',fontSize:14,fontFamily:'DM Sans,sans-serif',borderRadius:8,textAlign:'left',transition:'background 0.2s',background:'none',border:'none',cursor:'pointer'},
  main:{flex:1,marginLeft:268,minHeight:'100vh',overflow:'auto'},
};
